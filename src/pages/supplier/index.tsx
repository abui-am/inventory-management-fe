...
import SupplierDetailSheet from '@/components/supplier/SupplierDetailSheet';
...
const COLUMNS: Kolom[] = [
  ...
  { key: 'utang', label: 'Utang', align: 'right', width: '132px' },
...
const SupplierPage: NextPage & ThemeablePage = () => {
  ...
  const [customer, setCustomer] = useState<SupplierData | null>(null);
  const [sheetOpen, setSheetOpen] = useState(false);

  // `null` + dialog terbuka berarti membuat baru.
  const [mengubah, setMengubah] = useState<SupplierData | null>(null);
  ...
  const { data: dataSupplier, isLoading } = useFetchSuppliers({
  ...
  const bukaRincian = (row: SupplierData) => {
    setCustomer(row);
    setSheetOpen(true);
  };

  const bukaForm = (row: SupplierData | null) => {
    setMengubah(row);
    setFormOpen(true);
  };
  ...
  const kosong = useMemo(() => {
    if (debouncedSearch) {
      return {
        ...
        pesan: 'Pencarian membaca nama dan nomor HP supplier.',
      };
    }
    return { judul: 'Belum ada supplier', pesan: 'Supplier yang ditambahkan akan muncul di sini.' };
  }, [debouncedSearch]);
  ...
  return (
    ...
    <div className="flex flex-col gap-2.5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex w-full flex-wrap items-center gap-1.75 md:w-auto">
          ...
          <div className="min-w-0 flex-1 md:hidden">
            <ThemedSelect
              name="sortir"
              instanceId="sortir-supplier"
              aria-label="Urutkan supplier"
          ...
          <Button size="sm" onClick={() => bukaForm(null)}>
            <Plus strokeWidth={2.2} aria-hidden /> Supplier
          </Button>
        </div>
      </div>
                  ...
                  return (
                    <tr key={row.id} className="transition-colors duration-fast hover:bg-surface-raised">
                      ...
                      <td className={cn(TD, 'whitespace-nowrap text-right font-mono font-semibold tabular-nums')}>
                        {utang > 0 ? (
                          <span className="text-warning">{formatNumber(utang)}</span>
                        ) : (
      ...
      <SupplierFormDialog supplier={mengubah} isOpen={formOpen} onClose={() => setFormOpen(false)} />
    </div>
  );
};

SupplierPage.themeable = true;
